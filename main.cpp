...
int main() {
    ...
    if (bcount == 0) {
        ...
        b.setAll(0, rjup, 0, vjup,
                 msun); //hier müssen noch Anfangsbedingungen rein. Ort: 0 788e6, v 13000 0 masse 1.899e27
        c.setAll(rjup, 0, vjup, 0,
                 msun); //hier müssen noch Anfangsbedingungen rein. Ort: 788e6 0, v 0 -13000 masse 1.899e27
        ...
        for (unsigned i = 1; i < 10001; i++) {
            ...
            if (i % 100 == 0) {
                dat << a.getPosx(i) << " " << a.getPosy(i) << " " << b.getPosx(i) << " " << b.getPosy(i) << " "
                    << c.getPosx(i) << " " << c.getPosy(i) << endl;
    ...
    if (bcount == 1) {
        ...
        c.setAll(rerde-rmond,0,0,vmond, msun);
        a.print(0);
        ...
        for (unsigned i = 1; i < 10001; i++) {
            for (unsigned k = 0; k < bcount; k++) {
                (*body[k]).verPos(body, i, bcount);
            }
            if (i % 100 == 0) {
                data << a.getPosx(i) << " " << a.getPosy(i) << " " << b.getPosx(i) << " " << b.getPosy(i) << " " << c.getPosx(i) << " " << c.getPosy(i) << endl;
                //  dat << a.getPosx(i) << a.getPosy(i) << b.getPosx(i) << b.getPosy(i) << c.getPosx(i) << c.getPosy(i)  << endl;
            }
